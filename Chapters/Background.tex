...
\begin{figure}
    \centering\small
    \input{Figures/pss-hierarchy.tex}
...
\begin{itemize}
    \item \psst{Circumstance}:  Circumstantial complementary information of events (e.g., location, time, means, purpose)
    \item \psst{Participant}: Entity taking part in an event (e.g., agent, instrument)
    \item \psst{Configuration}: A static relationship between two entities (e.g., possession, comparison).
\end{itemize}

The 3 subtrees loosely correspond to adverbial adjuncts, event arguments, and adnominal complements, respectively. The \psst{Participant} and \psst{Circumstance} subtrees largely correspond to semantic relationships between a verb and its arguments. Many \psst{Circumstance} subtypes, however, like  \psst{Locus} (the location of an entity or event), can be governed by either a noun or a verb, both may or may not represent an event: \emph{work \textbf{at} the office, a meeting \textbf{at} the office, a desk \textbf{at} the office}. \psst{Configuration} covers other kinds of semantic relations, typically not dealing with space and time (e.g., possession, comparison). Each label covers its own range of semantic relations, that may be refined by a child label according to the SNACS hierarchy (no multiple inheritance). 

\cite{snacs} provide a general definition of SNACS labels by embedding them in the following scene prototypes: 

\begin{itemize}
\item Location - \psst{Theme} is located at \psst{Locus}
\item Motion - \psst{Theme} moves from \psst{Source} along \psst{Path} to \psst{Goal} 
\item Transitive Action - \psst{Agent} acts on \psst{Theme} , perhaps using an \psst{Instrument} 
\item Possession - \psst{Possesion} belongs to \psst{Possesor} 
\item Transfer - \psst{Experiencer} changes possession from \psst{Originator} to \psst{Recepient}, perhaps with \psst{Cost}
\item Perception - \psst{Experiencer} is mentally affected by \psst{Stimulus}
\item Cognition - \psst{Experiencer} contemplates \psst{Topic}
\item Communication - information (\psst{Topic}) flows from \psst{Originator} to \psst{Recipient}, perhaps via an \psst{Instrument}
\end{itemize}
...
Both sentences emit the same semantics of an employment relationship between the subject "Thomas" and the object of the preposition "MetaCortex". The corresponding SNACS label for such semantics is \psst{OrgRole}. On the other hand, one could argue that "at" in (1) implies a locational relationship, corresponds to the \psst{Locus} label (as evident from the fact that (1) is a valid answer for "Where does Thomas work?"). A similar overlap of preposition semantics is demonstrated in (2), as "for" also fits the notion of \psst{Beneficiary}: the employer is the beneficiary of the work done by the employee. 
...
Following the \rf{Role}{Function} notation, (1) would be annotated as \rf{OrgRole}{Locus} and (2) as \rf{OrgRole}{Beneficiary}.

A few more cases of role and function divergence are demonstrated in the following sentences:

\begin{itemize}
    \item Put the book \p{on}/\rf{Goal}{Locus} the shelf.
          
          "on" implies a locative sense (\psst{Locus}), yet in the established scene the location described is a destination (\psst{Goal}).
    \item I care \p{about}/\rf{Stimulus}{Topic} the environment.
          
          "the environment" triggers the caring action (\psst{Stimulus}) but "about" implies that it is also the \psst{Topic} of caring.
    \item The road runs \p{through}/\rf{Locus}{Path} the forest.
          
          This sentence specifies the location of the road (\psst{Locus}), yet "runs through" implies a motion, or \psst{Path}.
\end{itemize}
...
SkipGram and Continuous-Bag-Of-Words (CBOW) are two similar neural word embedding models jointly referred to as \textbf{Word2Vec} (\cite{word2vec}). They have perhaps been the first neural word embedding models to be both rather simple and fast to train, yet very successful at the task. Despite following improvements and progress made in recent years, they are still widely used nowadays due to their simplicity and effectiveness. Many pretrained Word2Vec models are publicly available, eliminating the need to train a model in order to obtain the word vectors. 
...
\subsubsection{As an isolated task} \label{sec:ppatt_isolated}
...
\subsection{PP Attachment in this work} \label{sec:ppatt_thiswork}
