...
While computing an exact solution for eq \ref{eq:bestparams} is often untraceable, there are many optimizers aiming to find a good estimation for the best set of parameters $\hat{\theta}$. Most of these optimizers are gradient-based, and are typically variations of the Gradient Descent (GD) minimization algorithm. GD is a iterative process aiming to minimize a differentiable function $t$. It begins with a randomly assigned $\theta$. In each iteration, the gradient $\nabla t = \frac{\partial t}{\partial \theta}$ is computed, and $\theta$ is updated accordingly:

$$ \theta = \theta - \textit{lr} \cdot \nabla t $$
...
SGD (along with its variations) is a heuristic process and does not guarantee convergence to a global minima. Emprical results consistently show, however, that neural networks can be trained successfully event when using plain SGD.
