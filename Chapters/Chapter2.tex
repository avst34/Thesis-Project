...
$$ p^{Func}_{start:end} = Softmax(MLP_{Func}(u_{start:end})) $$

$p^{Role}_{start:end}$ and $p^{Func}_{start:end}$ are then the corresponding predicted probability distributions for Role and Function labels respectively. The number of MLP layers and dimensions, as well as the activation functions used by the MLP layers, are all considered model hyperparameters. 
